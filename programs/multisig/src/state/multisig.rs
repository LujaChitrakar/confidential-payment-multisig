...
impl MultiSig {
    ...
    pub fn realloc_bytes(&self, change_type: &ChangeReallocType) -> usize {
        ...
        match change_type {
            ChangeReallocType::AddOwner {
                owner: _,
                stratum: _,
            } => req_len + 32,
            ChangeReallocType::AddStratum { stratum } => {
                req_len + 4 + stratum.owners.len() * 32 + 2 + 1
            }
        }
    }
    ...
    pub fn add_owner(&mut self, owner: Pubkey, stratum: u8) -> Result<()> {
        if self.is_owner_wallet(owner) {
            return Err(ErrorCode::OwnerAlreadyExists.into());
        }

        let target_stratum = &mut self.strata[stratum as usize];
        let target_owners = &mut target_stratum.owners;

        require!(target_stratum.active, ErrorCode::InactiveStratum);
        require_gt!(1220, target_owners.len(), ErrorCode::InvalidOwnersCount);
    ...
    pub fn remove_owner(&mut self, owner: Pubkey, s_index: usize) -> Result<()> {
        ...
        match owner_status {
            Some(index) => {
                require!(target_stratum.active, ErrorCode::InactiveStratum);
                require_gte!(
                    target_stratum.owners.len() - 1,
                    target_stratum.m as usize,
                    ErrorCode::ThresholdExceeds
                );
                target_stratum.owners.remove(index);
            }
            _ => {
                return Err(ErrorCode::OwnerDoesntExist.into());
            }
        }
    ...
    pub fn add_stratum(&mut self, owners: Vec<Pubkey>, m: u16) -> Result<()> {
        ...
        require_gt!(
            u8::MAX as usize,
            self.strata.len(),
            ErrorCode::InvalidStrataLen
        );
        require_gt!(owners.len(), 0, ErrorCode::InvalidOwnersCount);
        require_gte!(1220, owners.len(), ErrorCode::InvalidOwnersCount);

        require_gte!(owners.len(), m as usize, ErrorCode::ThresholdExceeds);

        for owner in owners.iter() {
            require!(!self.is_owner_wallet(*owner), ErrorCode::OwnerAlreadyExists);
        }
    ...
    pub fn deactivate_stratum(&mut self, s_index: usize) -> Result<()> {
        if s_index == 0 {
            return Err(ErrorCode::CannotDeactivateFirst.into());
        }

        let target_stratum = &mut self.strata[s_index];

        require!(target_stratum.active, ErrorCode::InactiveStratum);
        require_eq!(target_stratum.m, 0, ErrorCode::ThresholdNotZero);
    ...
    pub fn activate_stratum(&mut self, s_index: usize) -> Result<()> {
        require!(!self.strata[s_index].active, ErrorCode::ActiveStratum);
    ...
    pub fn change_m(&mut self, new_m: u16, s_index: usize) -> Result<()> {
        ...
        if s_index == 0 && new_m == 0 {
            return Err(ErrorCode::ThresholdZero.into());
        }
