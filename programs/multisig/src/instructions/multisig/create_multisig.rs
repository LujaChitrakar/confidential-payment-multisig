use crate::{
    error::ErrorCode,
    state::multisig::{MultiSig, Stratum},
    ID,
};
...
pub fn create_multisig_handler(
    ...
    require_gte!(25, name.len(), ErrorCode::InvalidNameLen);

    let strata_len = strata.len();
    require_gt!(strata_len, 0, ErrorCode::InvalidStrataLen);
    require_gte!(u8::MAX as usize, strata_len, ErrorCode::InvalidStrataLen);

    for (s_index, stratum) in strata_mut.iter_mut().enumerate() {
        let owners = &mut stratum.owners;
        let owners_len = owners.len();

        require_gt!(owners_len, 0, ErrorCode::InvalidOwnersCount);
        require_gte!(1220, owners_len, ErrorCode::InvalidOwnersCount);
        require_gte!(owners_len, stratum.m as usize, ErrorCode::ThresholdExceeds);

        if s_index == 0 {
            require_gt!(stratum.m as usize, 0, ErrorCode::ThresholdZero);
        }
            ...
            for (current_num, &current_key) in owners.iter().enumerate() {
                if s_index == c_index {
                    for &key in owners[current_num + 1..owners_len].iter() {
                        require_keys_neq!(current_key, key, ErrorCode::DuplicateOwner);
                    }
                } else {
    ...
    let (_authority, authority_bump) =
        Pubkey::find_program_address(&[b"authority", multi_sig.key().as_ref()], &ID);
